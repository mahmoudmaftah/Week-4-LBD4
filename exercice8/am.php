...
<html lang="en">
  ...
  <body>
    ...
    $a = 0; $b = 0;
    if( intval($_POST["salaire"])/2 > intval($mensualite) + intval($_POST["total"])){
        $b = 1;
    } else{
        if($_POST["salaire"] * 1/2 < $_POST["total"]){
            $prop = "Vous ne pouvez beneficier d'aucun service."
        }
        else{
            $prop = (1 - pow(1 + ($taux/1200), -$duree))*(1200/$taux)*(($_POST["salaire"]*52)/100 - intval($_POST["total"]));
        }

    }
    if(date("Y") + $_POST["duree"]/12 - intval(explode("-", $_POST["naiss"])[0]) < 62){
        $a = 1;
    }
